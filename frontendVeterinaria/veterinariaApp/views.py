...
def error_view(request, error_message, id):
    return render(
        request, "error_template.html", {"error_message": error_message}, {"id": id}
    )
...
def BuscarPacienteP(request, id):
    try:
        ...
        if response["Pacientes"]:
            ...
            messages.add_message(request, messages.SUCCESS, "Paciente Encontrado")
            return render(
                request, "poliza.html", {"id": id, "personas": response["Pacientes"]}
            )
        raise Exception(response["message"])
    except Exception as error:
    ...
    try:
        ...
        if respuesta.status_code == 200:
            messages.add_message(
                request, messages.SUCCESS, "Poliza del Paciente registrada"
            )
            return render(request, "administrador.html", {"id": id})
        else:
            raise Exception(str(response["message"]))
...
def BuscarEmpleado(request, id):
    try:
        ...
        if response["Empleados"]:
            ...
            return render(
                request,
                "actualizarEmpleado.html",
                {"id": id, "personas": response["Empleados"]},
            )
        raise Exception(response["message"])
    except Exception as error:
...
def renderLicenciaEmpleado(request, id):
    try:
        ...
        error_message = "Error en view front en def renderLicenciaEmpleado: " + str(
            error
        )
        return render(
            request, "error_template.html", {"id": id, "error_message": error_message}
...
def renderMuestraEmpleado(request, id):
    try:
        ...
        if response["Empleados"]:
            ...
            return render(
                request,
                "muestraEmpleado.html",
                {"id": id, "personas": response["Empleados"]},
            )
        raise Exception(response["message"])
    except Exception as error:
        error_message = "Error en view front en def renderMuestraEmpleado: " + str(
            error
        )
        return render(
            request, "error_template.html", {"id": id, "error_message": error_message}
    ...
    try:
        ...
        if response["Asistencias"]:
            ...
            return render(
                request,
                "asistencias.html",
                {"id": id, "personas": response["Asistencias"]},
            )
        raise Exception(response["message"])
    except Exception as error:
